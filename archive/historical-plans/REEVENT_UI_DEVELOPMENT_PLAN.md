# ReEvent UI Development and Integration Plan

## 1. Purpose

This document explains how to develop the ReEvent UI from the generated image boards while keeping the interface native, responsive, accessible, and suitable for a real Android app.

The UI is part of a production-oriented application: every user-facing action must connect through the shared navigation, ViewModel, and repository contracts. Supabase is the selected cloud backend, while Room and DataStore provide offline-first local storage.

The goal is a polished Jetpack Compose UI that presents real or repository-seeded data, handles loading/empty/error states, and remains usable when the device is temporarily offline.
...
- Repository-backed UI state for all runtime screen content.
- Fake repository seed data only for previews, automated tests, and deliberate demo fallback.
- Navigation Compose routes with typed arguments and real data operations.
...
## 4. UI Build and Integration Scope

### 4.1 Included

Build and integrate UI for:
...
The UI must integrate with the shared authentication, Supabase-backed repositories, Room cache, DataStore preferences, QR, maps, upload, and form-submission contracts as their owning tracks deliver them. UI work must not bypass these contracts with hard-coded success states or direct database access.

Preview-only sample data and temporary visual assets are allowed only when isolated from runtime flows and clearly replaceable by repository data or the relevant service.
...
- Use `map_partner_mock.png` only as a temporary offline/demo fallback until the real partner-map integration is available.
...
Use Navigation Compose with typed routes and stable entity IDs. Pass only route arguments required to load state; ViewModels obtain the data from repositories.

---

## 14. Seed and Preview Data Plan

Provide repository seed data for development/demo mode and separate preview fixtures for Compose previews. Runtime screens must not import a UI-level `MockData` object directly.
