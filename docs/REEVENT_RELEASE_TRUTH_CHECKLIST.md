...
**Current strict progress:** **11 / 168 release items accepted**, with **6 P0** and **9 P1** blockers recorded below. Stage 1 froze the release contracts; Stage 2A implemented and verified account-safe Room identity, scoped local mutations, schema export, and the version 1→2→3 migration path. This is intentionally stricter than feature-presence progress: existing partial functionality is described in Section 3 and earns acceptance only after its complete release scenario is evidenced.
...
| Offline/sync | Local-first outbox and WorkManager exist | Account-safe queues, fair retries, conflict/removal handling, visible status and recovery | Partial/blocking |
...
| Tests | 21 active local tests and one Room instrumentation test are present after archiving two unwired prototype-assessment tests | Unit, repository, policy-contract, UI, navigation, restoration, E2E, accessibility and release suites | Inadequate |
| Android build | Debug APK builds; local unit tests pass | Zero lint errors, signed/reproducible release AAB/APK, shrinker checked, release device matrix passed | Blocking |
...
- `:app:testDebugUnitTest` passes in the audited worktree.
- A debug APK exists at `ReEvent/app/build/outputs/apk/debug/app-debug.apk` and was approximately 69.6 MB during the audit.
- A prior connected run passed the single Room instrumentation test on an Android 17 emulator; it must be rerun for the final evidence set.
- Android lint currently fails with 3 errors and reports 29 warnings. The errors are an API-27 navigation-bar style used with minSdk 24, missing ML Kit `ExperimentalGetImage` opt-in, and CAMERA permission without a matching optional camera feature declaration.
...
- No authenticated three-account E2E, physical-camera QR run, live map run, offline conflict run, signed release build, or AppGallery test has been accepted.
...
|---|---|---|---|
| P0 | Transaction RLS lets actors insert/update critical fields and statuses too freely | `ReEvent/supabase/migrations/0001_core_platform.sql` | Replace client-trusting writes with constrained server functions/policies and test adversarial calls |
| P0 | Participant scan/return tries to mutate organiser-owned resources, passports, and impact rows that RLS rejects | `FeatureViewModel.kt`, `LocalFirstCoreRepository.kt`, migrations | Server-authoritative lifecycle function succeeds atomically for the authorised actor |
| P0 | The outbox is not filtered and scheduled by account/session | `CoreDao.kt`, `SyncScheduler.kt`, `LocalFirstCoreRepository.kt` | A queued operation can never execute as a different signed-in user |
| P0 | Multi-record lifecycle writes are not atomic | `FeatureViewModel.kt`, repository layer | One database/server transaction updates transaction, resource, passport history, and impact or updates none |
| P0 | Completion can be replayed or fabricated, producing duplicate/false impact | client workflow and migrations | Idempotency key, legal state machine, uniqueness constraints and replay tests |
...
| P1 | Android lint is red | Gradle lint result | Zero lint errors; reviewed warning baseline with no release-critical warnings |
...
- [ ] **REL-SYNC-01 — Bind outbox rows to an account and environment.** Worker execution must match the authenticated subject and backend.
...
- [ ] **REL-SYNC-05 — Protect sign-out/account switching.** Signing out stops work safely and a subsequent account cannot observe or execute the prior account's private queue.
