...
**Current strict progress:** **13 / 168 release items accepted**, with **5 P0** and **9 P1** blockers recorded below. Stage 1 froze the release contracts; Stage 2A established the account-safe Room cache; Stage 2B bound outbox execution and sign-out cleanup to the current account and typed local environment. Stage 3 is captured in reviewable commits `3721aa5efe0bfc597e4a2e02c0fee19bb561ec78` and `68e82033c45020e48d08db43259a04c86ae652a6`: they implement the frozen server-authoritative lifecycle, atomic completion, durable RPC idempotency, Room 5 projections, and the Supabase `pgcrypto` compatibility fix. Official staging SQL-actor evidence now passes, but no release item is accepted until real signed multi-account/device and lost-response evidence is recorded. This is intentionally stricter than feature-presence progress: existing partial functionality is described in Section 3 and earns acceptance only after its complete release scenario is evidenced.
...
| P0 | Replay/lost-response safety is proven locally but not yet in the staging interruption scenario | Postgres idempotency records; Room `lifecycle_commands`; retry test | Reuse the persisted UUID across a real interrupted response and prove no duplicate transaction, ledger, passport or impact effects |
